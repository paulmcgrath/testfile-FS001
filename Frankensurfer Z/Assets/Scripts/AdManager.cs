...
public class AdManager : Singleton<AdManager>, IRevMobListener {
	...
	private CBManager chartBoostManager;
	private AdType activeAdNetworks;

	void Awake() {
		if (useChartBoost) {
			chartBoostManager = FindObjectOfType<CBManager>() as CBManager;
	...
	void InitializeAds() {
		...
		if (useChartBoost) {
			...
			CBBinding.init();
			#elif UNITY_IPHONE
			CBBinding.init(chartBoostIosID, chartBoostIosSignature);
			#endif
...
#region Chartboost delegates
	...
	void OnEnable() {
		...
		if (useChartBoost) {
			...
			CBManager.didFailToLoadInterstitialEvent += ChartboostAdFailed;
			CBManager.didCloseInterstitialEvent += ChartboostAdClosed;
			CBManager.didCacheInterstitialEvent += ChartboostAdCached;
			CBManager.didShowInterstitialEvent += ChartboostAdShown;
			CBManager.didClickInterstitialEvent += ChartboostAdClicked;
		}
#endif
	}
