...
public class TerrainEditor : Editor {
	...
	public SerializedProperty Use2DCollider;
	...
	void OnEnable () {
		...
		Use2DCollider = terrainDisplayer.FindProperty("Use2DCollider");
	...
	public override void OnInspectorGUI () {
		...
		if (topPlaneFoldout){				
			...
			Use2DCollider.boolValue = EditorGUILayout.Toggle(new GUIContent("Use 2d Collider", "Uses Physics2D collider - for use with Unity's 2D tools"), Use2DCollider.boolValue);
        ...
        if (prefabGenerationRules)
        {
            ...
            for (int i = 0; i < PrefabRules.arraySize; i++)
            {
				...
				SerializedProperty randomOffset = prefabRule.FindPropertyRelative("RandomOffset");
                ...
                if (PrefabRulesExpanded[i])
                {
                    ...
                    if (!ruleDeleted)
                    {
						...
						EditorGUILayout.PropertyField(randomOffset, new GUIContent("RandomOffset", "Additional range for random offset."), true);
